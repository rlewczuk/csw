...
func TestResolveTaskNewPromptSkipsTaskCreationWhenEditedPromptIsEmpty(t *testing.T) {
	...
	runTaskEditorFunc = func(ctx context.Context, editorCommand string, promptFilePath string) error {
		_ = ctx
		_ = editorCommand
		assert.True(t, strings.HasPrefix(filepath.Clean(promptFilePath), filepath.Join(workDir, ".cswdata", "tmp")+string(os.PathSeparator)))
		return os.WriteFile(promptFilePath, []byte(" \n\t "), 0o644)
	}

	prompt, shouldCreate, err := resolveTaskNewPrompt(context.Background(), taskNewPromptParams{Editor: "editor", WorkDir: workDir})
	require.NoError(t, err)
...
func TestResolveTaskNewPromptReturnsEditedPrompt(t *testing.T) {
	...
	workDir := t.TempDir()
	tmpDir := filepath.Join(workDir, ".cswdata", "tmp")
	...
	runTaskEditorFunc = func(ctx context.Context, editorCommand string, promptFilePath string) error {
		_ = ctx
		_ = editorCommand
		assert.True(t, strings.HasPrefix(filepath.Clean(promptFilePath), tmpDir+string(os.PathSeparator)))
		info, err := os.Stat(filepath.Dir(promptFilePath))
		require.NoError(t, err)
...
func TestResolveTaskNewPromptUsesShadowDirForTemporaryPromptFile(t *testing.T) {
	originalEditorRunner := runTaskEditorFunc
	t.Cleanup(func() {
		runTaskEditorFunc = originalEditorRunner
	})
	...
	workDir := t.TempDir()
	shadowDir := t.TempDir()
	shadowTmpDir := filepath.Join(shadowDir, ".cswdata", "tmp")

	runTaskEditorFunc = func(ctx context.Context, editorCommand string, promptFilePath string) error {
		_ = ctx
		_ = editorCommand
		assert.True(t, strings.HasPrefix(filepath.Clean(promptFilePath), shadowTmpDir+string(os.PathSeparator)))
		return os.WriteFile(promptFilePath, []byte("prompt from shadow\n"), 0o644)
	}

	prompt, shouldCreate, err := resolveTaskNewPrompt(context.Background(), taskNewPromptParams{Editor: "editor", WorkDir: workDir, ShadowDir: shadowDir})
	require.NoError(t, err)
	assert.True(t, shouldCreate)
	assert.Equal(t, "prompt from shadow", prompt)
}
