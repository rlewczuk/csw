...
func TestRunTaskArchiveDefaultStatusMerged(t *testing.T) {
	...
	require.NoError(t, err)
	completedTask, err := manager.CreateTask(core.TaskCreateParams{Name: "completed", NoCommit: true, Prompt: "prompt"})
	require.NoError(t, err)
	completedWithBranchTask, err := manager.CreateTask(core.TaskCreateParams{Name: "completed-with-branch", Prompt: "prompt"})
	require.NoError(t, err)
	...
	setTaskStatusForTest(t, filepath.Join(baseDir, ".cswdata", "tasks", completedTask.UUID, "task.yml"), core.TaskStatusCompleted)
	setTaskStatusForTest(t, filepath.Join(baseDir, ".cswdata", "tasks", completedWithBranchTask.UUID, "task.yml"), core.TaskStatusCompleted)
	...
	assert.Contains(t, buffer.String(), completedTask.UUID)
	assert.NotContains(t, buffer.String(), completedWithBranchTask.UUID)
	...
	_, completedErr := os.Stat(filepath.Join(baseDir, ".cswdata", "tasks", "archive", completedTask.UUID, "task.yml"))
	require.NoError(t, completedErr)
	_, completedWithBranchErr := os.Stat(filepath.Join(baseDir, ".cswdata", "tasks", completedWithBranchTask.UUID, "task.yml"))
	require.NoError(t, completedWithBranchErr)
