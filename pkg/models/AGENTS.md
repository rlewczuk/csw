...
* `ollama_client.go` - Ollama provider client implementation core (client setup, listing, and sync chat).
* `ollama_client_stream.go` - Ollama chat streaming implementation and streaming raw-chunk logging helper.
...
* `ollama_client_stream_test.go` - Ollama streaming chat behavior and streaming-logging tests split from the main Ollama client test file.
