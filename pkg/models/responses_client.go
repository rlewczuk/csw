...
func (c *ResponsesChatCompactor) CompactMessages(messages []*ChatMessage) ([]*ChatMessage, error) {
	if c == nil || c.chatModel == nil || c.chatModel.client == nil {
		return nil, fmt.Errorf("ResponsesChatCompactor.CompactMessages() [responses_client.go]: chat model not initialized")
	}
	if len(messages) == 0 {
		return nil, fmt.Errorf("ResponsesChatCompactor.CompactMessages() [responses_client.go]: messages cannot be nil or empty")
	}
	if c.chatModel.model == "" {
		return nil, fmt.Errorf("ResponsesChatCompactor.CompactMessages() [responses_client.go]: model not set")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("ResponsesChatCompactor.CompactMessages() [responses_client.go]: failed to marshal request: %w", err)
	}
	...
	executeRequest := func(token string) (*http.Response, []byte, error) {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("ResponsesChatCompactor.CompactMessages() [responses_client.go]: failed to create request: %w", err)
		}
		req.Header.Set("Content-Type", "application/json")
		if token != "" {
			req.Header.Set("Authorization", "Bearer "+token)
		...
		bodyBytes, err := io.ReadAll(resp.Body)
		if err != nil {
			readErr := fmt.Errorf("ResponsesChatCompactor.CompactMessages() [responses_client.go]: failed to read response body: %w", c.chatModel.client.handleHTTPError(err))
			return nil, nil, wrapLLMRequestError(readErr, resp, nil)
		}
	...
	if err := json.Unmarshal(bodyBytes, &compactResp); err != nil {
		decodeErr := fmt.Errorf("ResponsesChatCompactor.CompactMessages() [responses_client.go]: failed to decode response: %w", err)
		return nil, wrapLLMRequestError(decodeErr, resp, bodyBytes)
	}
