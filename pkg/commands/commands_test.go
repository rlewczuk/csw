...
func TestLoadFromDirParsesCSWMetadata(t *testing.T) {
	...
	require.NotNil(t, command.Metadata.CSW.Parameters)
	assert.False(t, command.Metadata.CSW.Parameters.Merge)
	assert.Equal(t, 0, command.Metadata.CSW.Parameters.MaxThreads)
	assert.Equal(t, "", command.Metadata.CSW.Parameters.Model)
...
func TestLoadFromDirFallsBackToEmbeddedCommand(t *testing.T) {
	...
	assert.Contains(t, command.Template, "Analyze and edit current task description for errors, ambiguities and missing parts.")
	assert.Contains(t, command.Template, "Results of analysis should be saved in current task description using `taskEdit` tool")
