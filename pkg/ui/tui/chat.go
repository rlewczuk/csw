...
type ChatWidget struct {
	...
	// Program reference for sending update messages (used for triggering re-renders)
	program *tea.Program
...
func (w *ChatWidget) AddMarkdownChunk(markdown string) {
	...
	// Get program reference before unlocking
	p := w.program
	w.mu.Unlock()

	// Send update message to trigger re-render (must be done after unlocking
	// to avoid deadlock since Send blocks until Update processes the message)
	if p != nil {
...
// SetProgram sets the bubbletea program reference for sending update messages.
// This should be called after creating the program but before starting it.
func (w *ChatWidget) SetProgram(p *tea.Program) {
	w.program = p
}
