...
type SessionOutputHandler interface {
	// AttachToSession is called to associate given output handler with session.
	AttachToSession(session *SweSession)
...
func (s *SweSystem) NewSession(model string) (*SweSession, error) {
	...
	if s.UiFactory != nil {
		...
		session.outputHandler.AttachToSession(session)
	}
