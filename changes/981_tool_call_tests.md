In `anthropic_client_test.go` implement additional integration tests for tool calling:
...
* it should use model `claude-sonnet-4-5-20250929` for testing;
* use some sample tool call and response, for example weather tool;
* as other tests, it should be run only if anthropic integration tests are enabled (see `pkg/testutil/integ.go`);
