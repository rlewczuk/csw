In `openai_client_test.go` implement additional integration tests for tool calling:
...
* as other tests, it should be run only if openai integration tests are enabled (see `pkg/testutil/integ.go`);
